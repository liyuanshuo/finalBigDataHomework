...
if __name__=="__main__":
    ...
    lines = sc.textFile("/liyuanshuo/knnData_5000.data", 1)
    ...
    print('***' * 15 + "KNN的准确率" + '***' * 15)
    print("这是KNN算法的准确率： " + str(result.count() * 1. / (lines.count() - trainingSize)))
    print('***' * 15 + "KNN的准确率打印完成" + '***' * 15)
    sc.stop()
    end=time.time()
    print('***' * 15 + "打印程序运行时间：单位（s）" + '***' * 15)
    print(str(end-begin))
    print('***' * 15 + '***' * 15)